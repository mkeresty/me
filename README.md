...
npm run icons      # regenerate the favicon set into app/
```

The favicon is a squared M under the hero's scan bar, generated by [`scripts/build-icons.mjs`](scripts/build-icons.mjs). The SVG adapts to the browser's colour scheme; the PNGs can't, so they use the dark variant. Icons are **numbered** (`icon0.svg`, `icon1.png`) so Next emits an explicit `<link>` for each — a bare `favicon.ico` gets probed at the domain root, which sits outside the basePath on a project-page deploy and would 404.
